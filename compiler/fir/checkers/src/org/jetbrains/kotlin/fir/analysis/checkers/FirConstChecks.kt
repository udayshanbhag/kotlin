...
internal fun checkConstantArguments(
    ...
    when {
        ...
        expression is FirFunctionCall -> {
            ...
            if (symbol.fir.annotations.none { it.isFoldableAnnotationCall }) return ConstantArgumentKind.NOT_CONST
        ...
        expression is FirQualifiedAccessExpression -> {
            ...
            when {
                property.unwrapFakeOverrides().annotations.any { it.isFoldableAnnotationCall } -> {
                    return checkConstantArguments(expression.dispatchReceiver, session)
                }
